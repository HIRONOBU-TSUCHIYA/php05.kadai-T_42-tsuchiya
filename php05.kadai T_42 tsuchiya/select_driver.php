...
if ($status==false) {
  ...
  while( $result = $stmt->fetch(PDO::FETCH_ASSOC)){
    $view2 .= "<tr>";
    $view2 .='<td>'. h($result['date']) .'</td>'
    .'<td>'. h($result['driver_name']).'</td>' .'<td>'.h($result['company_name']) 
    .'</td>'.'<td>'.h($result['client']).'</td>'.'<td>'.h($result['tell']).'</td>'
    .'<td>'.h($result['start_place']).'</td>'.'<td>'.h($result['end_place']).'</td>'
    .'<td>'.h($result['item']).'</td>';
    // クロスサイトスクリプティング対策
    $view2 .= '<td>'.'<a href="detail_dr.php?id='. $result['id'] .'">';
    $view2 .= '[更新]';
    $view2 .= '</a>'.'</td>';
    $view2 .= '<td>'.'<a href="delete.php?id='. $result['id'] .'">';
    $view2 .= '[削除]';
    $view2 .= '</a>'.'</td>';
    $view2 .= "</tr>";
  }
